...
filter_par_n0      = 0.d0
T_EP_eV            = 1.75d5    ! EP temperature in eV (default 175 keV)
RHO_EP             = 2.5d18     ! Total number of EP particles
particle_pusher    = 'gc_rel'  ! Particle pusher, e.g. gc_rel, kin_rel
Lambda_peak        = -1         ! it's negative, so by default we don't use that.
delta_Lambda       = 0.3d0   
output_pe_E_mu     = .false.    ! output power exchange in (psi_N, E, mu)
output_pe_vpar_mu  = .false.    ! output power exchange in (psi_N, mu, P_phi)
use_pe_EmuPphi     = .false.    ! output power exchange in (E, mu, P_phi) (independent flag)
pdf_A              = 0.604693d0 ! skewed-normal amplitude (default d13)
pdf_xi             = 0.2580d0  ! skewed-normal location
